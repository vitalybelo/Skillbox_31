...
public:
    IGraph() = default;
    explicit IGraph(IGraph *other) {};

    virtual void AddEdge(int from, int to) = 0; // Метод принимает вершины начала и конца ребра и добавляет ребро
    [[nodiscard]] virtual int VerticesCount() const = 0; // Метод должен считать текущее количество вершин и возвращать его
    [[nodiscard]] virtual std::vector<int> getVerticesNumbers() const = 0; // Возвращает список с номерами вершин, которые существуют в графе
